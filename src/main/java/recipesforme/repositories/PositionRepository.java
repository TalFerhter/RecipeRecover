...
public interface PositionRepository extends CrudRepository<Position, UUID> {
    ...
    @Query(value = "select p from Position p where p.pos_id = ?1 and p.recipe_id = ?2 and p.row = ?3")
    ...
    @Query(value = "select p from Position p where p.row = ?1 and p.col = ?2 and p.paragraph = ?3")
    Iterable<Position> findWordPositions(int row, int col, UUID paragraph_id);

    @Query(value = "select p from Position p where p.recipe_id = ?1 and p.row = ?2 and p.col = ?3")
    Iterable<Position> findWordPositionsByRecipe(UUID recipe_id, int row, int col);

    @Query(value = "select p from Position p where p.recipe_id = ?1 and p.row = ?2 and p.col = ?3 and p.paragraph = ?4")
